...
t_maxlen	get_max_lengths_files(t_file *files)
{
	...
	while (!(S_ISDIR(files->s_stat->st_mode)))
	{
		stats = files->s_stat;
		if (max_len.len_hlinks < \
				(unsigned int)count_digits(stats->st_nlink, 10))
		...
		if (max_len.len_bytes < (unsigned int)count_digits(stats->st_size, 10))
...
void		sort_files(t_file **files, t_flag flag)
{
	...
	{
		if (flag & FLAG_UBIG)
			sort_lst(*files, cmp_flag_ubig, flag & FLAG_R ? 1 : 0);
		else if (flag & FLAG_U)
			sort_lst(*files, cmp_flag_u, flag & FLAG_R ? 1 : 0);
		else
